...
namespace cartesian_planner
{
  ...
  std::tuple<double, double, double> DpPlanner::GetCost(StateIndex parent_ind, StateIndex cur_ind)
  {
    double parent_s = state_.start_s, grandparent_s = state_.start_s;
    double parent_l = state_.start_l, grandparent_l = state_.start_l;
    double parent_v = state_.start_vel, grandparent_v = state_.start_vel;
    ...
    double cost_longitudinal_velocity = fabs(cur_v - config_.dp_nominal_velocity);
    double cost_longitudinal_velocity_change = fabs(dv1 / unit_time_);
    double attenuation = 0.5, dp_w_longitudinal_velocity_stablity = 2.0;
    double cost_longitudinal_velocity_stablity = fabs((cur_v - state_.start_vel) * std::pow(attenuation, cur_ind.t));

    double delta_cost = (config_.dp_w_lateral * cost_lateral +
                         ...
                         dp_w_longitudinal_velocity_stablity * cost_longitudinal_velocity_stablity);

    return std::make_tuple(cur_s, cur_v, delta_cost);
  }

  bool DpPlanner::Plan(double start_x, double start_y, double start_theta, double start_vel, DiscretizedTrajectory &result)
  {
    auto sl = env_->reference().GetProjection({start_x, start_y});
    state_.start_s = sl.x();
    state_.start_l = sl.y();
    state_.start_theta = start_theta;
    state_.start_vel = start_vel;
    ...
    for (int i = 0; i < NS; i++)
    {
      for (int j = 0; j < NL; j++)
      {
        ...
        state_space_[0][i][j].current_s = std::get<0>(tup);
        state_space_[0][i][j].current_vel = std::get<1>(tup);
        state_space_[0][i][j].cost = std::get<2>(tup);
      }
    }
        ...
        for (int k = 0; k < NL; k++)
        {
          ...
          for (int m = 0; m < NS; m++)
          {
            for (int n = 0; n < NL; n++)
            {
              ...
              double cur_s = std::get<0>(tup);
              double cur_vel = std::get<1>(tup);
              double delta_cost = std::get<2>(tup);

              double cur_cost = state_space_[i][j][k].cost + delta_cost;
              if (cur_cost < state_space_[i + 1][m][n].cost)
              {
                state_space_[i + 1][m][n] = StateCell(cur_cost, cur_s, cur_vel, j, k);
              }
            }
          }
        }
    ...
    for (int i = 0; i < NT; i++)
    {
      ...
      for (int j = 0; j < nseg_; j++)
      {
        ...
        data[n].velocity = waypoints[i].second.current_vel;
      }
    }
