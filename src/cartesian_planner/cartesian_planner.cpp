...
namespace cartesian_planner
{
  ...
  bool CartesianPlanner::Plan(const StartState &state, DiscretizedTrajectory &result)
  {
    ...
    if (!dp_.Plan(state.x, state.y, state.theta, state.v, coarse_trajectory))
    //if (!dp_.Plan(state.x, state.y, state.theta, 0, coarse_trajectory))
    {
      ROS_ERROR("DP failed");
      return false;
    }
    TrajectoryToFile(coarse_trajectory, "/tmp/dp_traj");
    ...
    TrajectoryToFile(result, "/tmp/result_traj");
    return true;
  }

  void CartesianPlanner::TrajectoryToFile(const DiscretizedTrajectory &trajectory, std::string path)
  {
    std::ofstream o(path);
    std::vector<TrajectoryPoint> data = trajectory.data();
    for (auto p : data)
    {
      o << p.velocity << " ";
    }
