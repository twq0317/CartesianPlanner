...
namespace cartesian_planner
{
  ...
  public:
    ...
    bool Plan(double start_x, double start_y, double start_theta, double start_vel, DiscretizedTrajectory &result);
  ...
  private:
    struct StateCell
    {
      ...
      StateCell(double cost, double cur_s, double cur_vel, int parent_s_ind, int parent_l_ind)
          : cost(cost), current_s(cur_s), current_vel(cur_vel), parent_s_ind(parent_s_ind), parent_l_ind(parent_l_ind) {}
    };
    ...
    std::tuple<double, double, double> GetCost(StateIndex parent_ind, StateIndex cur_ind);
